...
- **This pass:** the BUG-030 iteration-2 doc-fold applied here (`bug030-iter2-revalidation` entry above, `ISS-F2V-013`/`ISS-F2V-023` status updates + `ISS-F2V-024`/`ISS-F2V-025` filed + weak-point register extended per `CVR-012` in `workflow_discussion_f1f2.md`, checklist rows for `EXP-018`/`BUG-036`/`BUG-037`) is a further monitoring-doc checkpoint on top of `eeb6439`/`d68c8a2`/`266eea1` — not yet committed. Prior pass: the W7b/W8 doc-fold (`w7b-main-matrix` + `w8-adjudication-disposition` entries, `ISS-F2V-013`..`ISS-F2V-022` + the weak-point register, checklist flips for W7b/W8) plus the BUG-030 iteration-1 fold (`bug030-fix-revalidation` entry).
- **Uncommitted:** this plan doc (`docs/ai/validation_plan_f1f2_continuous.md`), `docs/ai/golden_labels_f1f2.md`, and all three monitoring docs (`workflow_checklist_f1f2.md`, `workflow_results_f1f2.md`, `workflow_discussion_f1f2.md`, including this pass's iteration-2 fold) remain uncommitted pending the user's word. `docs/ai/development_report.md` does not exist in the current worktree — its BUG-030 iteration-2 entry is deferred to post-blind, parked in `bug030-iter2-revalidation`'s own DR-equivalent block above.
