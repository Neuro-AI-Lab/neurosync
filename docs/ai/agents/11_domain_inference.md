...
| **역할** | RAG 기반 정신건강 영역(domain)/진료과(department) 후보 추론. **ADR-041 이후 이 에이전트가 시스템 전체에서 유일하게 LLM 호출을 수행하는 RAG 관련 에이전트다** — 아래 "ADR-041 통합" 절 참조 |
...
## ADR-041 통합 (2026-07-20)

**단일 RAG surface.** ADR-041 이후 이 에이전트는 RAG 경로에서 LLM을 호출하는 유일한 컴포넌트다.
Policy-B LLM judge arm(`RagTriggerJudgeAgent`, 구 agent 15)은 폐기되었다 — agent 모듈, 스키마,
프롬프트, `agent_model_registry.yaml` entry가 모두 live 코드베이스에서 삭제되었다(스펙은
`_archive/legacy_code/agent_inventory_wave6/15_rag_trigger_judge.md`에 archived; 검증:
`grep -r "rag_trigger_judge" apps/ai-server/src`가 0 hits를 반환하며,
`apps/ai-server/src/rag_trigger.py`의 모듈 docstring은 이제 "RAG trigger policy A —
PLAN-2026-W28-Q W4"로 표기되고 `TriggerDecision.policy`가 `Literal["A"]`로 narrowing되어
single-arm이 됨).

**통합된 live 체인(3단계, LLM 호출 1회):**

```
src/rag_trigger.py::decide_policy_a   (code-side, LLM 없음 — trigger 결정 + risk-lexicon filter)
    ▼
f2.py::run_stage1                     (in-process retrieval, src/rag/retrieval.py)
    ▼
DomainInferenceAgent.run (이 에이전트) (유일한 RAG LLM 호출 — Stage 2, 이전과 동일)
    ▼
f2_grounding.py evidence cascade      (code-side, LLM 없음 — source-id whitelist + risk-lexicon filter)
```

`f2.py`는 더 이상 `--rag-trigger-policy` CLI 플래그를 받지 않는다 — `decide_policy_a`가 무조건
호출된다(검증: `grep "rag_trigger_policy" apps/ai-server/src/f2.py`가 0 hits 반환; `f2.py`는
`src.rag_trigger`에서 `decide_policy_a`/`no_rag_decision`만 import하며 `decide_policy_b`는 절대
import하지 않음). 이 절 아래의 나머지 모든 내용(Stage 1/Stage 2 아키텍처, 4계층 evidence-verification
방어선, risk-vocabulary filter, 프롬프트 pin, 라우팅, API 라우트, 인증 상태, 알려진 결함)은 이
에이전트 자체의 Stage-2 LLM 호출을 기술하며, ADR-041로 변경되지 않았다 — 이를 먹이는 trigger
layer만 단일 code-side arm으로 통합되었을 뿐이다.
...
이 에이전트(11)의 RAG top-5 disease 후보를 기반으로 하는 새 비진단·비임상 필드다. `f2.py`가 이를 기존 `domain_candidates` 출력에 **병합하지 않고** sibling key로 배선한다 — 즉 F2 산출물에 나란히 존재하는 별개 컨테이너다.
