...
| ID | Type | Item | Status | Preconditions/notes |
|---|---|---|---|---|
| T1-F2-SEC-004 | SEC | Track A: RAG HTTP API permanent descope — `rag_router` unmounted from `main.py` (RETIRE, not DORMANT, per `REV-013` §2/`ADR-017`), `rag_chat.py` refactored to call `retrieve_grounding()` in-process, `src/rag/route.py`/`auth.py` retired-not-deleted (fresh ADR + VAL-005 review required before remounting) | [x] | User decision (permanent, categorical — no HTTP-served RAG now or at deployment). qa live-verified 2026-07-09: `POST /ai/rag/grounding` → 404 (route absent), `/health` → 200, zero rag-prefixed mounted paths. `VAL-005` structurally resolved (not merely mitigated). `UPSTAGE_API_KEY` confirmed disjoint, unaffected. Source: `error.md` VAL-005, `discussion.md` ADR-017/REV-013 §2, `development_report.md` DR-010 §2 |
