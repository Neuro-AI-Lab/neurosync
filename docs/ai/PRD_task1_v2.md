...
**상태: 설계 v1 완료 (pre-implementation) — 구현 전. 어떤 검증·구현 완료 서술도 포함하지 않는다.** 설계 원문: `_archive/plans/f3_quick_dev_plan.md`(근거 기록은 `discussion.md` `PLAN-2026-W28-V`/`ADR-031`).
...
- **기록 (F5 소비 대상):** VP별·세션별 원장(`<persona_id>_session_ledger.json`)에 `"f3"` 하위 객체로 기록되도록 설계됨 — 아티팩트/원장 스키마 상세는 `_archive/plans/f3_quick_dev_plan.md` §4/§5.
- **Item bank v0 (허구 생성 금지 원칙 준수):** PHQ-9/AUDIT-C만 persona 파일 출처의 구성개념(construct) 라벨로 v0 populated(`provenance="construct-labels-v0, persona-file-sourced, non-validated"`); GAD-7/PHQ-4/WHO-5는 저장소 전역에 문항 원문·응답 anchor가 부재하여 v0에서 unpopulated로 남는다(`_archive/plans/f3_quick_dev_plan.md` §2). v1 item bank(검증된 문항 원문)는 사용자 결정 대기(같은 문서 §2.3).
...
근거: `discussion.md` PLAN-2026-W28-V, ADR-031; `_archive/plans/f3_quick_dev_plan.md`.
