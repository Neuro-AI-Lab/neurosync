...
| # | 격차 | 처분 (ADR-013) | 상태 |
|---|---|---|---|
| S1 | `rag_chat.py:33` 공인 IP:포트 리터럴 + `:36-42` 환자 UUID 4종 하드코딩 — `POST /ai/rag/grounding` 무인증이라 UUID가 사실상 접근 자격증명. git 이력 2커밋, `origin/Master` 포함 | **옵션 (a) 채택**(2026-07-08): rag 라우터에 env 기반 bearer/API-key 인증 적용(타 라우트 무영향) + `rag_chat.py` 하드코딩 IP/UUID → env/인자화. **갱신(2026-07-09, `PLAN-2026-W28-H` Track A, `ADR-017`): 처분이 인증(mitigation)에서 RETIRE(제거)로 변경됐다** — RAG는 현재·배포시 모두 in-process only이며 HTTP로 서빙할 계획이 없다는 사용자의 영구·범주적 결정에 따라, 완화가 아니라 노출면 자체의 제거를 선택했다(critic `REV-013` §2 판정) | **구현 완료·검증됨** (`T1-F2-SEC-001`, 2026-07-08 시점) — `src/rag/auth.py`(fail-closed 503/미설정, 401/403 오류키, 200 정상키, dev-mode bypass, 타 라우트 무영향), `tests/rag/test_route_auth.py` 6 cases, qa GATE:PASS. `rag_chat.py` 하드코딩 IP/UUID 제거 확인. **갱신(2026-07-09):** `rag_router`가 `main.py`에서 언마운트되어 `POST /ai/rag/grounding` 자체가 더 이상 라이브 앱에 존재하지 않는다 — qa 라이브 확인: 404(라우트 부재), `/health` 200(타 라우트 무영향), rag-접두 경로 0건. `src/rag/auth.py`/`route.py`는 retired-not-deleted로 트리에 남아 있다(재마운트 시 신규 ADR + VAL-005 재검토 필수). **`VAL-005`는 구조적으로 해소**(무인증 엔드포인트 자체가 소멸 — 영구 완화가 아니라 제거). `NS_RAG_API_KEY`는 현재 미사용. 근거: `discussion.md` ADR-017, REV-013 §2; `error.md` VAL-005; `development_report.md` DR-010 §2 |
...
**미결 (옵션 (b) 미승인):** git 이력 재작성(하드코딩 IP/UUID가 이미 `origin/Master`에 커밋된 상태를 소급 제거)은 **미승인** — 조율된 force-push 결정이 필요하며 미결로 유지한다. 인증 도입 후 위험도는 하락하나(무인증 상태 해소), 이력상 노출 자체는 잔존한다. **갱신(2026-07-09):** Track A RETIRE 처분(위 표 S1 갱신 참조) 이후 라이브 노출면은 완전히 소멸했으나, 이 git 이력 항목 자체는 별개로 여전히 미결이다.
...
**게이팅:** 컨테이너는 지금 구축됐다(`mode="experimental_unpopulated"`). 라이브 실채움은 RAG-arm 인증에 게이트된다 — RAG arm은 현재 **EXPERIMENTAL/UNCERTIFIED**로 잔류(`ADR-016`, 이번 미션으로 변경 없음)해, 실채움 대상이 될 라이브 데이터 흐름 자체가 아직 없다. leakage 감사는 `DATASET-005` 애든덤(2026-07-09, `data`) 참조 — 4/6 항목이 design/provenance review로 종결, 2/6 미결(쿼리측 순환성은 F2 자체의 기존 미결 항목을 상속, ontology-graph 경로의 "Ada" 분류체계 출처는 신규 미결).

근거: `discussion.md` PLAN-2026-W28-H, REV-013 §3/§4, DATASET-005 애든덤; `error.md` BUG-019 "Track B" subsection; `development_report.md` DR-010 §3.
