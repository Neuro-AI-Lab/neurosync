...
**Status:** open — iteration-1 fix landed (`dd4eba2`), re-validated (`EXP-017`), NOT resolved to the pre-registered bar; iteration-2 fix landed (`d68c8a2`), re-validated (`EXP-018`), repetition bar (criterion A) **FAILS** as a pre-registered bar, mission stop-rule fired, no iteration-3 without user word

**Owning gate:** qa

**Cross-refs:** `BUG-030`, `CVR-009` (naturalness/badgering read), `REV-030`, `EXP-017`, `ADR-028`, `CVR-010`, `REV-031`, `BUG-035`/`ISS-F2V-023`, `EXP-018`, `ADR-029`, `REV-032`, `REV-033`, `CVR-011`, `CVR-012`, `BUG-036`/`ISS-F2V-024`, `BUG-037`/`ISS-F2V-025`
...
**Owning gate:** qa (filed) + critic (`REV-033` independently corroborated the defect and its measurement-vs-mechanism distinction)

**Cross-refs:** `BUG-036`, `REV-033`, `CVR-012`, `EXP-018`, `BUG-030`/`ISS-F2V-013`, `ADR-029`
...
**Owning gate:** qa (`BUG-037` filed) + clinical-validator (`CVR-012` F1/F2)

**Cross-refs:** `BUG-037`, `BUG-029` (lineage — first confirmed shipped hit in that entry's open class), `CVR-012`, `EXP-018`
...
| 15 | Detected-but-shipped on retry-budget exhaustion — a caught safety-adjacent naturalness violation ships to the patient rather than degrading safely (VP-003 141607, budget exhausted 2/2, "shipped to the patient rather than degraded safely") | MAJOR | `CVR-012` F4, `ISS-F2V-013` (`BUG-030`) |
