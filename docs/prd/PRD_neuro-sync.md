...
- **v1.5 (2026-07-15)**: 프론트엔드 v3 방향 반영 — 증분 PRD [`PRD_frontend_v3.md`](./PRD_frontend_v3.md) 신설 (FR-038~049)
  ...
  - **위험 트리아지 개정**: "위험 감지 시 `/emergency` 강제 전환" 폐지 → **공통 위험 확인 창**(대화=모달·문진=인라인)에서 [도움 받기] 선택 시에만 응급 진입. §2.2 Gherkin·§5.4.1·Flow C 갱신
  - **핫라인 기준 변경**: 1393(자살예방상담전화) → **109(자살예방 통합번호, 2024-01 통합)**. §2.2·FR-011·§5.1 payload 예시·Flow A/C·Appendix D 갱신
  - **리포트 전달 개정**(FR-018 → v3 FR-047): 자동 생성·전달 → 보관 → 환자 열람·수정 → 수동 [전달하기]
  - 데모 범위: v3 P0(FR-039~044)를 7/31 데모에 포함 (2026-07-15 결정) — 상세 일정은 v3 PRD §7
...
- **위험 발화 감지 시 즉시 안전 안내 및 응급 라우팅**(119/109)으로 전환하는 안전망을 시스템 레벨에서 내재화한다.
...
- As a **불안·우울 증상이 심한 사용자**, I want to **위기 순간에 즉각적인 응급 연락처(109, 119) 안내를 받고 싶다**.
...
**Scenario: 위험 발화 감지 시 확인 후 라우팅** *(v1.5 개정 — v3 FR-042: 강제 전환 폐지, 사용자 확인 후 진행)*
```
Given 환자가 사전 문진 채팅 중이다
When 환자가 자살·자해 의도를 시사하는 메시지를 입력한다
Then 일반 대화 흐름이 즉시 중단되고
And 위험 확인 창(모달)이 표시되며
And 환자가 [도움 받기]를 선택한 경우에만 위험 신호 대응 화면으로 전환되고
And 119, 자살예방 통합번호 109 안내가 표시되고
And "지금 혼자 있나요?" 최소 안전 확인 질문이 표시되고
And [괜찮아요] 선택 시 대화로 복귀하되
And 두 경우 모두 서버에 위험 이벤트가 로그로 저장된다
...
| **위기 보호 통보 정책 (C-1 자문 결과 반영 잠정안)** | 환자 회원가입 시 **위험 통보 동의(FR-026)** 옵트인을 받음. 동의 시: high/critical 위험 감지 → 비상 연락처 SMS 안내 + 의료진 대시보드 플래그. 자동 119/109 통보는 **환자에게 안내만** (자동 신고는 자살예방법 §14 자문 후). 옵트아웃 시: 환자 본인에게 핫라인 안내 화면만 표시. **법률 자문 결과에 따라 v1.2에서 확정.** |
...
| `/intake/chat` | ✓ | - | ✓ | ✓ | - | **위험 감지 시 위험 확인 창(모달) 표시 → [도움 받기] 선택 시에만 `/emergency` 진입 (v1.5, v3 FR-042)** / **음성 동의 시 마이크 버튼 노출(FR-033), 녹음 중 `recording` 상태(파형+경과시간), 변환 중 `transcribing` 상태, 결과는 편집 가능 영역에 노출 후 명시적 전송(FR-035), 실패 시 키보드 폴백(FR-037)** |
...
flowchart TD
  ...
  Chat -->|일반 발화| Chat
  Chat -.->|위험 발화 감지| RiskModal["위험 확인 창 (모달, v3 FR-042)"]
  RiskModal -->|도움 받기| Emergency["/emergency"]
  RiskModal -.->|괜찮아요| Chat
  Emergency -->|"119/109 안내 + 안전 확인 → [안전해요]"| Chat
  Chat -->|"대화 종료 · RAG 도메인 추정(백그라운드, v3 FR-039)"| Loading["'분석 중' 로딩 (v3 FR-041)"]
  Loading --> Survey["top1 문진 1종 (문항 주입형, v3 FR-040)<br/>우울→PHQ-9 · 불안→GAD-7 · 음주→AUDIT-C · 폴백→PHQ-4"]
  Survey --> Submit["/intake/submit 최종 검토"]
  Submit -->|제출| Status["/report/status 생성 중"]
  Status -->|30초 후| Done["리포트 보관 — 환자 수동 전달 (v3 FR-047)"]
  Done --> Home
```

> **v1.5 개정**: PHQ-9→GAD-7 고정 순차 및 사전 문진 단계 문서 업로드(`/intake/documents`)는 폐지. 문서(처방전)는 대화 중 [+] 첨부로 이동 (v3 FR-048). 상세는 [`PRD_frontend_v3.md`](./PRD_frontend_v3.md) §2.
...
flowchart TD
  ...
  Block --> Confirm["위험 확인 창 (v1.5, v3 FR-042)"]
  Confirm -.->|괜찮아요| Stream
  Confirm -->|도움 받기| EmergencyUI["/emergency 화면"]
  EmergencyUI --> Hotline["119 / 109 / 비상 연락처 표시"]
...
- [ ] 위험 등급 분류 + `/emergency` 화면 — FR-011 *(v1.5: 강제 전환 → 위험 확인 창 경유로 개정, v3 FR-042)*
...
**데모 IN** *(v1.5 개정 — v3 P0 포함 결정, 2026-07-15)*: 환자 가입(4개 동의)·로그인, AI 채팅, Safety Guard + **위험 확인 창 → emergency** (v3 FR-042~044), **RAG top1 단일 문진** (v3 FR-039~041: PHQ-9/GAD-7/AUDIT-C + 폴백, 문항 주입형 컴포넌트, 플랫폼 프록시 + questionnaire type 확장 포함), Handoff Report 생성 + 의료진 뷰, STT(Whisper 모드).
...
### D. 위험 신호 대응 화면 핫라인 *(v1.5: 109 기준으로 통일 — 1393·1577-0199는 2024-01부터 109로 통합)*
