"""PHR ingest util — myhealthway PHR raw JSON → PhrSummary.
...
ADR-041: demoted from `src.agents.patient_history.PatientHistoryAgent` (an
`AgentInput`/`BaseAgent`-shaped clinical agent) to this plain module of
functions. PHR is a code-side pre-load at session start from a
caller-supplied JSON path, never an LLM-facing agent call — it never
belonged in the BUG-022 clinical-agent-input allowlist surface. Callers
that need an `efficacy_adapter` (HIRA API fallback on cache miss) pass it
explicitly; nothing here is a singleton or holds mutable instance state.
...
async def _resolve_efficacy(
    code: str, efficacy_adapter: HiraDrugEfficacyAdapter | None
) -> EfficacyInfo | None:
    """캐시 hit 우선, miss & 어댑터 존재 시 HIRA API 조회."""
    info = hira_efficacy_cache.lookup(code)
    if info is not None:
        return info
    if efficacy_adapter is not None:
        return await efficacy_adapter.get_efficacy(code)
    return None


async def _classify_medications(
    medications: list[MedicationEvent],
    efficacy_adapter: HiraDrugEfficacyAdapter | None,
) -> None:
    ...
    for m in medications:
        ...
        if code not in resolved:
            resolved[code] = await _resolve_efficacy(code, efficacy_adapter)
        info = resolved[code]
        if info is None:
            continue
...
async def load_bundles(
    paths: list[str | Path], reader: MyHealthWayReader | None = None
) -> list[dict[str, Any]]:
    """여러 PHR 파일 순차 로드. 파일별 에러는 개별 로깅 후 스킵."""
    reader = reader or MyHealthWayReader()
    bundles: list[dict[str, Any]] = []
    for p in paths:
        try:
            bundles.append(await reader.load_bundle(p))
...
async def summarize(
    ...
    *,
    ...
    for bundle in bundles:
        for resource in reader.iter_resources(bundle):
    ...
    await _classify_medications(medications, efficacy_adapter)
...
async def run(
    ...
    *,
    ...
    """`PhrLoadInput.bundle_paths` 파일들을 병합 요약 (구 `PatientHistoryAgent.run`)."""
    started = time.perf_counter()
    bundles = await load_bundles(inp.bundle_paths, reader=reader)
    summary = await summarize(bundles, reader=reader, efficacy_adapter=efficacy_adapter)
...
async def run_from_bundles(
    bundles: list[dict[str, Any]],
    *,
    efficacy_adapter: HiraDrugEfficacyAdapter | None = None,
) -> PhrSummary:
    """`run()`'s in-memory counterpart — bundles arrive already parsed as
    request-body JSON objects (e.g. `POST /ai/phr/context`), never touching
    disk. `summarize()` itself is already file-I/O-free (`MyHealthWayReader.
    iter_resources` operates on a plain dict); this wrapper only adds the
    same latency/reason_summary enrichment `run()` gives the path-based
    flow, so both entry points return an equally complete `PhrSummary`.
    """
    started = time.perf_counter()
    summary = await summarize(bundles, efficacy_adapter=efficacy_adapter)
    summary.latency_ms = (time.perf_counter() - started) * 1000.0
    summary.reason_summary = (
        f"parsed {len(bundles)} bundle(s) (in-memory), "
        f"{summary.total_medication_events} meds "
        f"({len(summary.psychotropic_medications)} psychotropic), "
        f"{summary.total_visits} visits"
    )
...
def to_system_prompt_note(summary: PhrSummary) -> str:
...
def to_handoff_snippet(summary: PhrSummary) -> dict[str, Any]:
...
def _cli() -> int:
    ...
    ap = argparse.ArgumentParser(description="PHR ingest CLI (dev)")
    ap.add_argument("--file", "-f", action="append", required=True,
                    help="PHR JSON 파일 경로 (여러 번 사용 가능)")
    args = ap.parse_args()

    async def main() -> None:
        bundles = await load_bundles(args.file)
        summary = await summarize(bundles)
        print("=" * 60)
        ...
        print(to_system_prompt_note(summary))
        print()
