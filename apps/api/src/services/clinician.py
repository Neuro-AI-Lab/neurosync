...
def _can_access_patient(actor: User, patient_hospital_id: uuid.UUID | None) -> bool:
    """PRD §0.1 access rule (ISS-022).

    A clinician / org_admin may read ONLY patients of their own organization
    (``User.organization_id == PatientProfile.target_hospital_id``); a
    super_admin (service_role) may read all.
    """
    if actor.role == "super_admin":
        return True
    if actor.organization_id is None:
        return False
    return patient_hospital_id is not None and patient_hospital_id == actor.organization_id


def _patient_list_filter(actor: User):
    """WHERE clause scoping a patient query to *actor*'s org, or None for super_admin."""
    if actor.role == "super_admin":
        return None
    if actor.organization_id is None:
        return false()
    return PatientProfile.target_hospital_id == actor.organization_id


async def list_patients(
    db: AsyncSession, *, actor: User, limit: int
    ...
    org_filter = _patient_list_filter(actor)  # ISS-022: org-scope the dashboard
    if org_filter is not None:
        stmt = stmt.where(org_filter)
    rows = await db.execute(stmt.order_by(desc(User.created_at)).limit(limit))
...
async def get_patient_detail(
    db: AsyncSession, *, actor: User, patient_id: uuid.UUID
) -> PatientDetail | None:
    ...
    if pair is None:
        return None
    user, profile = pair
    # ISS-022: org-scope — treat out-of-org patients as not found (404).
    if not _can_access_patient(actor, profile.target_hospital_id):
        return None
