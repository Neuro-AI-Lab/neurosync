#!/usr/bin/env bash
# EXP-011 launch script — clean populated-path re-verification (REV-017 Finding 1 fix),
# VP-001~004 n=2 each, RAG mode (k=3 default), genuinely new live LLM calls.
set -u
...
run_one () {
  ...
  local outdir="$REPO_ROOT/experiments/EXP-011/runs/$vp/$run"
  mkdir -p "$outdir"
